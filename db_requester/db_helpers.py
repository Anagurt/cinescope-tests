...
class DBHelper:
    ...
    def user_count_by_email(self, email: str) -> int:
        ...
        """
        return (self.db_session.query(UserDBModel).filter(UserDBModel.email == email).count())
    ...
    def movie_count_by_name(self, name: str) -> int:
        ...
        """
        return (self.db_session.query(MovieDBModel).filter(MovieDBModel.name == name).count())
